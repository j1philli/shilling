...
if [ -d "$sdk_root/platforms/android-37" ] && [ -d "$sdk_root/build-tools/36.0.0" ]; then
    echo "Android SDK 37 and build tools 36.0.0 are installed"
    exit 0
fi
...
"$sdkmanager" --sdk_root="$sdk_root" "platforms;android-37" "build-tools;36.0.0"
